...
-(id) init
{
	...
	if( (self=[super init])) {
        ...
        if([prefs integerForKey:@"bestScore"] > 0)
        {
            best = [prefs integerForKey:@"bestScore"];
        }
        ...
        lblCounter.position = ccp(size.width/2, size.height/2);
...
-(void) showCountDown: (ccTime) dt
{
    if(countDown == 0)
    {
        [self removeChild:lblCounter cleanup:YES];

        isStarted = true;
        
        [self unschedule: @selector(showCountDown:)];

    }
    else
    {
        [lblCounter setString:[NSString stringWithFormat:@"%i", countDown]];
        countDown--;
    }
}
...
-(void) nextFrame:(ccTime)dt
{
    ...
    else if(isStarted == true)
    {
        ...
        {
            best = score;
            [prefs setInteger:best forKey:@"bestScore"];
            [prefs synchronize];
        }
...
{        
    ...
    lblCounter = [CCLabelTTF labelWithString:@"3" fontName:@"Marker Felt" fontSize:150];
    lblCounter.position = ccp(size.width/2, size.height/2);

    
    [self addChild:lblCounter];
    
    countDown = 3;
    
    [self schedule: @selector(showCountDown:) interval:1];
}
